/**
 * 貸款評估全流程：層次說明展開後，各項解說直接顯示於卡片內
 * 路徑：/demo/loan-process
 */
import { Link } from "wouter";
import { ArrowLeft } from "lucide-react";
import ProcessFlowContent from "@/components/ProcessFlowContent";
import { loanProcessSteps } from "@/data/processSteps";
    ...
    <div className="min-h-screen pb-20">
      <div className="border-b border-border bg-muted/30">
        <div className="container py-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-foreground mt-4">貸款評估全流程</h1>
          <p className="text-muted-foreground text-sm mt-1">依步驟展開，即可在卡片內查看各項說明。</p>
        </div>
      </div>
      <div className="container py-6">
        <ProcessFlowContent steps={loanProcessSteps} />
      </div>
    </div>
  );
}
